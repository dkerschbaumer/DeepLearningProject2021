...
def findBestRegularizer(best_hyperparameters, x_train_dist, x_train_clean, x_val_dist, x_val_clean):
    regularizers = {
        'reg1': [None],  # ['l1', 'l2', 'l1l2'],
        'weight1': [None],  # [0.01, 0.04, 0.1, 0.3],
        'reg2': [None],  # ['l1', 'l2', 'l1l2'],
        'weight2': [None],  # [0.01, 0.04, 0.1],
    }
    regularizers = list(ParameterGrid(regularizers))
    comb_list = []

    for regularizer in regularizers:
        reg1 = regularizer['reg1']
        weight1 = regularizer['weight1']
        reg2 = regularizer['reg2']
        weight2 = regularizer['weight2']

        autoencoder = Autoencoder(neurons=64, activation='relu', input_size=28)
        autoencoder.createDenseModel(best_hyperparameters['nr_layers'], best_hyperparameters['loss_func'], regularizer)
...
def createAndTrainModel(best_hyperparameters, x_train_dist, x_train_clean, regs=None):
    autoencoder = Autoencoder(neurons=64, activation='relu', input_size=28)

    if best_hyperparameters['layer_type'] == 'dense':
        autoencoder.createDenseModel(best_hyperparameters['nr_layers'], best_hyperparameters['loss_func'], regs)
    else:
...
def main():
    ...
    # evaluation y_test_clean = pd.read_csv('data/test.csv').iloc[:, 0].to_numpy() # id of image
    ...
    # "distorted x" should be predicted as "y clean" create validation set
    ...
    if USE_SAVED_MODEL:
        ...
        best_reg = findBestRegularizer(best_hyperparameters,
                                       x_train_dist, x_train_clean, x_val_dist, x_val_clean)

        autoencoder = createAndTrainModel(best_hyperparameters, x_train_dist_full, x_train_clean_full, regs=best_reg)
